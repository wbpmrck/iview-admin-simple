...
validate.setValidateFunction("existInTable", {
    fn:function (db,table,cols, cb) {
        ...
        db.models[table].count({where:condition}).then(function (count) {
            cb&&cb(count>0);
        }).catch(function (err) {
            cb&&cb(false,err.toString());
        });
    },
    desc:'必须在表{2}中存在' //使用{index}来引用函数的输入参数
});
...
validate.setValidateFunction("notExistInTable", {
    ...
    desc:'必须不在表{2}中' //使用{index}来引用函数的输入参数
});

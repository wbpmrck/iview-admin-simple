...
module.exports=async (ctx,next)=>{
    ...
    //判断哪些路由不需要进行登录检查
    if( (matched && matched.path && matched.path.length>0 && !matched.path[0].name)|| req.path.indexOf(".")>-1){
        logger.debug("指定路由不做检查");
        await next();
    }else if(!matched || matched.path.length<1){
        resp.status =404;
    }else if(matched && matched.path && matched.path.length>0 && matched.path[0].name !== undefined ){
